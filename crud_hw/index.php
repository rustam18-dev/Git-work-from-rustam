...
<div class="books">
    ...
    <asideR class="asideR">
        ...
        <hr>
        <p><b>Автор: </b><?= htmlspecialchars($row['author'])?></p>
        <hr>
        <p><b>Дата публикации:</b> <?=htmlspecialchars($row['public_of_year'])?></p>
        <hr>
        <p><b>Артикл:</b> <?=htmlspecialchars($row['isbn'])?></p>
        <hr>
        <p><b>Язык:</b> <?=htmlspecialchars($row['lang'])?></p>
        <hr>
        <p><b>Описание:</b> <?=htmlspecialchars($row['description'])?></p>
    </asideR>
    <?php endwhile; ?>
</div>
...
<div class="containers">
    <form method="post" action="action/action.php" class="form" enctype="multipart/form-data">
        ...
        <div class="mb-1 col-7">
            ...
            <input type="text" class="form-control" name="public_of_year" value="<?= htmlspecialchars($public_of_year)?>">
        </div>
        <div class="mb-1 col-7">
            <label  class="form-label">Артикл</label>
            <input type="number" class="form-control" name="isbn" value="<?= htmlspecialchars($isbn)?>">
        </div>
        <div class="mb-1 col-7">
            <label  class="form-label">Язык</label>
            <input type="text" class="form-control" name="lang" value="<?= htmlspecialchars($lang)?>">
        </div>
        <div class="mb-1 col-7">
            <label class="form-label">Изображение</label>
            <input type="file" class="form-control" name="img_url" value="<?= htmlspecialchars($img_url)?>">
        </div>
